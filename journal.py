...
def close_trade(trade_id, result, exit_price, pips_result, notes="", reason="MANUAL"):
    ...
    for t in journal:
        if t["id"] == trade_id and t["status"] == "OPEN":
            ...
            t["closed_at"] = datetime.now(tz_jkt).strftime("%Y-%m-%d %H:%M WIB")
            _save_journal(journal)
            return True
    return False
...
def _close_synced(trade, result, exit_price, pips, candle_date, reason):
    """Helper to close a trade during sync (modifies dict in-place)."""
    trade["status"] = result
    ...
    trade["notes"] = f"Auto-sync: {reason} pada {candle_date}"
